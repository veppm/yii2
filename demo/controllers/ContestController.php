...
class ContestController extends Controller
{
    ...
    {
        $query = new Query;
        $transactions = $query->select([ 'c.*', 't.transaction_id', 't.transaction_amount','t.payment_status'])
                    ->from('transaction_details as t')
                    ->InnerJoin('contest as c', 't.contest_id=c.id')
                    ->where(['c.user_id'=>\Yii::$app->user->getId()])
                    ->orderBy(['t.payment_date' => SORT_DESC])
                    ->all();
        return $this->render('my-transactions', ['transactions'=>$transactions]);
    }
